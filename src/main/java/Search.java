...
public class Search {
  ...
  public static String getVideos(String text) {
    Properties properties = new Properties();
    try {
      InputStream in = Search.class.getResourceAsStream("/" + PROPERTIES_FILENAME);
      properties.load(in);
    ...
    try {
      ...
      if (searchResultList != null) {
        return print(searchResultList.iterator(), queryTerm);
      }
    } catch (GoogleJsonResponseException e) {
      System.err.println("There was a service error: " + e.getDetails().getCode() + " : "
              + e.getDetails().getMessage());
  ...
  private static String print(Iterator<SearchResult> iteratorSearchResults, String query) {
StringBuffer buffer = new StringBuffer();
    buffer.append(
            "   First " + NUMBER_OF_VIDEOS_RETURNED + " videos for search on \"" + query + "\".");

    if (!iteratorSearchResults.hasNext()) {
      buffer.append(" There aren't any results for your query.");
    }
    ...
    while (iteratorSearchResults.hasNext()) {
      ...
      if (rId.getKind().equals("youtube#video")) {
        ...
        buffer.append(" Title: " + singleVideo.getSnippet().getTitle());
        buffer.append(" Thumbnail: " + "https://youtube.com/watch?v="+rId.getVideoId());
        buffer.append("\n-------------------------------------------------------------\n");
      }
    }
